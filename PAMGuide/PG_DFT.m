...
% CORRECTED 13 December 2021 to address bug in calculation of one-third
...
% Debug 13/12/2021
Pss = Pss(:,flow:fhigh);            %FIX to ensure Pss covers correct   
                                    %   frequency range during TOL analysis
% -----
...
% Debug 13/12/2021 - changing Pss(:,flow:fhigh) to Pss since this is now
% enacted on line 113

switch atype                  
    case 'PSD'                      %if PSD selected (EQUATION 11)
        a = 10*log10((1/(delf*B))*Pss./(pref^2))-S;  
    case 'PowerSpec'                %if power spectrum selected
        a = 10*log10(Pss./(pref^2))-S;  %EQUATION 10
    case 'Broadband'                %if broadband level selected
        a = 10*log10(sum(Pss,2)./(pref^2))-S;
                                    %EQUATION 17
% -----                                    
